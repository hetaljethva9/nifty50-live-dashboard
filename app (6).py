...
Live status logic (per stock, per request):
  ...
  Each flip is further annotated per the dual-timeframe strategy in the
  user's trading notes: each stock's current 15-min Supertrend (10,3) is
  computed as the "macro bias," and a flip is flagged "aligned_with_15m"
  if its direction matches that bias, plus "after_945" (the strategy's
  rule to wait for the first 15-min candle to close before acting).
  strategy_signal = "valid_entry" only when both are true. This is a
  Supertrend confirmation signal only -- it does not replicate the
